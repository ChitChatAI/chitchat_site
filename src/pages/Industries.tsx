...
const staggerChildren = { show: { transition: { staggerChildren: 0.08 } } };
const tileReveal = {
  hidden: { opacity: 0, y: 12 },
  show: { opacity: 1, y: 0, transition: { duration: 0.5, ease: [0.22, 1, 0.36, 1] } },
};

// Tailwind helpers (symmetry + equal height)
const CARD_BASE = [
  'group relative flex h-full flex-col justify-start',
  'rounded-md border border-white/10 bg-black/60 shadow-sm backdrop-blur',
    ...
    >
      {(icon || badge) && (
        <div className="mb-6 flex items-center gap-4">
          {icon && <div className="grid h-12 w-12 place-items-center rounded-md bg-theme-main/15">{icon}</div>}
...
const SectionHeader: React.FC<{ eyebrow?: string; title: string; desc?: string }> = ({ eyebrow, title, desc }) => {
  ...
  return (
    <motion.div className="text-center mb-14" variants={fadeInUp}>
      ...
      <h2 className="mt-4 text-3xl sm:text-4xl font-extrabold tracking-tight">{title}</h2>
      {desc && (
        <p className="mx-auto mt-0 max-w-3xl text-white/90 text-base sm:text-[1.0625rem] md:text-[1.125rem] leading-7 sm:leading-8">
          {desc}
        </p>
      )}
    </motion.div>
...
const Industries: React.FC = () => {
  ...
  useEffect(() => {
    const t = setInterval(
      () => setCurrentImageIndex((i) => (i === carouselImages.length - 1 ? 0 : i + 1)),
      5000
    );
  ...
  const additionalSolutions = [
    { title: 'Insurance', text: 'Quote-to-claim journeys that reduce cognitive load and build trust with clear, empathetic explanations at each step.' },
    { title: 'Retail Banking & Fintech', text: 'Behavioral nudges and transparent guidance for disputes, limits, and budgeting—without jargon or anxiety spikes.' },
    { title: 'Startups & SMB', text: 'Ship support that feels human from day one. Keep headcount low while scaling consistency and quality.' },
    { title: 'Ride-Hailing & Logistics', text: 'Real-time driver/rider help with calm conflict-resolution scripts and loyalty-building follow-ups.' },
    { title: 'Travel & Hospitality', text: 'Concierge-style itineraries, rebooking flows, and proactive updates delivered in a reassuring voice.' },
    { title: 'Banking & Finance', text: 'Explain transactions and loans plainly. Personas maintain trust through empathy, clarity, and secure patterns.' },
  ];

  const useCases = [
    { title: 'Psychology-Driven Support Automation', desc: 'Lower handle time and raise CSAT with intent detection, tone-mirroring, and de-escalation baked into every reply.', icon: <MessageCircle className="h-6 w-6 text-white" /> },
    { title: 'Sales Guidance & Lead Qualification', desc: 'Qualify by motivation and readiness, then guide decisions with choice framing and social proof—not pressure.', icon: <Brain className="h-6 w-6 text-white" /> },
    { title: 'Appointments, Reminders & Follow-Through', desc: 'Reduce no-shows using behavioral nudges, commitment devices, and timely check-ins that feel personal.', icon: <HeartPulse className="h-6 w-6 text-white" /> },
    { title: 'AI-Powered Tutoring & Coaching', desc: 'Adaptive scaffolding, formative feedback, and positive reinforcement to sustain momentum and mastery.', icon: <BookOpen className="h-6 w-6 text-white" /> },
  ];
  ...
  return (
    <>
      <SeoHelmet title={seoConfig.title} description={seoConfig.description} keywords={seoConfig.keywords} path={seoConfig.path} />

      <main ref={containerRef} className="font-aeonik bg-gradient-to-br from-gray-950 to-gray-950 text-white">
        {/* Optional soft brand wash */}
        <div
          aria-hidden
          className="pointer-events-none absolute inset-0 -z-10 bg-[radial-gradient(80%_60%_at_50%_0%,#260a40/18%,transparent_60%)]"
        ...
        >
          <div className="mx-auto max-w-7xl">
            ...
            <motion.div
              className="grid grid-cols-1 md:grid-cols-2 gap-8 [grid-auto-rows:minmax(0,1fr)]"
              variants={staggerChildren}
              initial="hidden"
              whileInView="show"
        ...
        >
          <div className="mx-auto max-w-7xl">
            ...
            <motion.div
              className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 [grid-auto-rows:minmax(0,1fr)]"
              variants={staggerChildren}
              initial="hidden"
              whileInView="show"
              viewport={{ once: true, amount: 0.2 }}
