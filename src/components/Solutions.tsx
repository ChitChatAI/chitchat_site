...
import NavBar from './purpleNavBar';
      ...
      <main ref={parallaxRef} className="relative z-10 overflow-hidden">
        <section id="hero" className="relative pt-48 pb-32 px-6 overflow-hidden">
          ...
          >
            ...
            <div className="mb-24">
              <div className="flex items-center justify-center">
                <h2 className="text-4xl font-bold text-gray-900">
                  Built for Every Business.<br />Designed to Feel Human.
                </h2>
              </div>
            ...
            {/* Category Cards - Removed scroll animations */}
            <div id="categories" className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-24 text-left">
              {solutionCategories.map((category, index) => (
                <div
                  key={index}
                  className={`scroll-review bg-white/80 backdrop-blur-sm rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden no-underline`}
        ...
        <section id="industries" className="relative bg-white py-32 px-6 overflow-hidden">
          <div className="max-w-6xl mx-auto">
            ...
            {/* Futuristic display with sophisticated design */}
            <div className="relative space-y-14">
              {additionalSolutions.map((item, index) => (
                <div
                  key={index}
                  className={`relative group ${index % 2 === 0 ? 'md:flex md:flex-row' : 'md:flex md:flex-row-reverse'}`}
                >
                  {/* Content card with futuristic styling */}
                  <div className="p-6 md:p-8 rounded-xl bg-gradient-to-br from-gray-50 via-white to-gray-100 backdrop-blur-md shadow-lg hover:shadow-2xl transition-all duration-300 relative overflow-hidden">
                    {/* Neural dots */}
                    <div className="absolute top-0 left-0 w-4 h-4 bg-theme-main rounded-full animate-float"></div>
                    <div className="absolute bottom-0 right-0 w-4 h-4 bg-theme-main rounded-full animate-float-slow"></div>

                    {/* Title and description */}
                    <h4 className="text-2xl font-bold text-theme-main mb-3 group-hover:text-theme-dark transition-colors duration-300">
                      {item.title}
                    </h4>
                    ...
                    <div className="absolute inset-0 bg-gradient-to-br from-transparent to-theme-main/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"></div>
                  </div>
                </div>
              ))}
            </div>
